...
public class Nave1TankControl : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKey(KeyCode.S)) movimento = -1f;

        float rotacao = 0f;
        if (Input.GetKey(KeyCode.A)) rotacao = 1f;
        if (Input.GetKey(KeyCode.D)) rotacao = -1f;
